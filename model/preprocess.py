...
def get_data(test):
    ...
    d = ["お仕事No.","動画ファイル名","掲載期間　終了日","期間･時間　備考",'休日休暇(金曜日)', '大量募集', '短時間勤務OK(1日4h以内)']
    test.drop(d,axis = 1,inplace = True)
    
    return test,no


def prerpocess_run(file):
    test,no = get_data(file)
    te = onehot(test)
